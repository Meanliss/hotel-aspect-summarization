"""Run aspect_inference.py in parallel across entity shards, then merge.

Each shard runs in a separate Python process writing to its own
run_id folder; afterwards the per-aspect files are copied into a single
...
def build_cmd(args, shard_idx, shard_run_id):
    return [
        ...
        "--sentencepiece", str(DATA_DIR / "sentencepiece" / "hasos_unigram_32k.model"),
...
def merge_shards(shard_run_ids, final_run_id):
    ...
    for shard_run_id in shard_run_ids:
        ...
        if not shard_dir.exists():
            print(f"[merge] WARNING: shard dir missing: {shard_dir}")
            continue
