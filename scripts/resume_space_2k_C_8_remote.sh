...
LOG=logs/space_2k_C_8_resume.log
echo "=== RESUME start: $(date -Is) ===" | tee "$LOG"
...
/workspace/venv/bin/python train.py \
  ...
  --epochs 8 \
  --start_epoch 4 \
  --resume_model /workspace/SemAE/checkpoints/space_2k_C_8/space_2k_C_8_4_model.pt \
  --batch_size 5 \
  --d_model 320 \
  --codebook_size 1024 \
  --nlayers 3 \
...
echo "=== RESUME end: $(date -Is) ===" | tee -a "$LOG"
