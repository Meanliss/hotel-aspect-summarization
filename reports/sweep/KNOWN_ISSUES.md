...
| HASOS threshold (M2/M3/M4 × ≤0.005) | ⚠️ **đang dở + có bug đã fix code nhưng CHƯA rerun** |
...
## LỖI #1 — M2 HASOS sweep score ra 0 (BUG, đã fix code, chưa verify)

**Triệu chứng:** mọi cell `rouge_m2_hasos_threshold_*.json` đã sinh đều cho
...
**Fix đã áp vào `scripts/sweep_params.py` (CHƯA chạy lại để verify):**
...
**Việc còn phải làm để đóng lỗi #1:**
1. Smoke-test 1 cell: `sweep_params.py --dataset hasos --phase threshold --grid 0.005 --methods m2`
   → kỳ vọng macro R1 ≈ **0.20015** (full pool = baseline). Đang chạy thì bị dừng tay.
2. Nếu khớp → rerun đủ HASOS M2 grid {0.0, 0.0025, 0.004, 0.005} (mỗi cell thêm parent pass, lâu hơn child-only).
3. Các cell HASOS M2 sai đã bị xoá khỏi `reports/sweep/` rồi (không còn file rác).
